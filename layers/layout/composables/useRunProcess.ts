...
interface UseRunProcessOptions {
  graph: Ref<any> // eslint-disable-line @typescript-eslint/no-explicit-any
  cancelOnError?: Ref<boolean> | boolean
}
...
export function useRunProcess({ graph: dagreGraph, cancelOnError = true }: UseRunProcessOptions) {
  ...
  async function runNode(nodeId: string, isStart = false) {
    ...
    upcomingTasks.add(nodeId)    // get all incoming edges to this node
   ...
   */
  async function run(nodes: { id: string }[]) {
   ...
   */
  function reset(nodes: { id: string }[]) {
    clear()
